import path from "path";
import { readJson, writeJson } from "../../../../utils/json.js";

export const answerUrlClean = async (dir, url) => {
  const inputFile = path.join(dir, "url-list.json");
  const outputFile = path.join(dir, "url-list-answer-clean.json");

  const urlList = await readJson(inputFile);
  const { problemSetProblems } = urlList;

  // 截取到 last-submissions 为止，拼接查询参数
  const baseUrl = url.match(/.*\/last-submissions/)?.[0];
  if (!baseUrl) {
    throw new Error("Invalid URL format");
  }

  const urlListClean = problemSetProblems.map(
    (item) => `${baseUrl}?problem_set_problem_id=${item.id}`
  );

  await writeJson(outputFile, urlListClean);
  console.log(`Done: ${outputFile}`);
  return urlListClean;
};

// CLI entry
if (process.argv[1].includes("answer-url-clean")) {
  const dir = process.argv[2];
  const url = process.argv[3];
  if (!dir || !url) {
    console.error("Usage: node answer-url-clean.js <data-dir> <url>");
    process.exit(1);
  }
  answerUrlClean(dir, url);
}
